...
def load_dataset(dataset_directory, text_line_image_to_text_mapping, image_width, image_height_with_padding,
    ...
    y_train = np.ones([size, absolute_max_string_length]) * -1  # , dtype=np.int8) * -1
    y_lenghts = [0] * size
    pool_size = 2
    ...
    for i, file in enumerate(files):
        line_image = lines_to_window_converter.read_grayscale_image_and_add_padding(
            os.path.join(dataset_directory, "lines-training", file), image_height_with_padding)
...
if __name__ == "__main__":
    ...
    configuration = ConfigurationFactory.get_configuration_by_name("simple", image_width, image_height, 77,
                                                                   absolute_max_string_length)
    print(configuration.summary())
    model = configuration.model()

    inputs, outputs = load_dataset(dataset_directory, text_line_image_to_text_mapping, image_width, image_height,
                                   absolute_max_string_length)
    ...
    model_checkpoint = ModelCheckpoint(best_model_path, verbose=1)
    callbacks = [model_checkpoint]

    model.fit(x=inputs, y=outputs, batch_size=configuration.training_minibatch_size,

...
def split_dataset_into_training_validation_and_test_sets(dataset_directory: str = "data", training_precentage=0.8,
                                                         test_percentage=0.2):
    ...
    # Corresponds to a 80%, 20% split for train and test
    test_pairs = randomized_files[0:math.ceil(len(randomized_files) * test_percentage)]
    training_pairs = randomized_files[len(test_pairs):len(randomized_files)]

    print("Split a total of {0} samples into two sets with {1} training and {2} test samples.".format(
        len(randomized_files), len(training_pairs), len(test_pairs)
    ))
...
if __name__ == "__main__":
    split_dataset_into_training_validation_and_test_sets("data")
